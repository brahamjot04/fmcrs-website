...
(function($,_,Backbone,Drupal){"use strict";var Selection=Backbone.View.extend({events:{'click .views-row':'onClick','dblclick .views-row':'onClick'},renderCounter:function(){$('.media-browser-file-counter').remove();if(this.count){var $counter=$('<div class="media-browser-file-counter"></div>');var $viewContent=$('.view-content');var text=Drupal.formatPlural(this.count,'1 item selected.','@count items selected.');$counter.text(text).addClass('messages messages--status');$viewContent.prepend($counter);}},initialize:function(){this.uuid=this.el.getAttribute('data-entity-browser-uuid');var settings=(frameElement?parent:window).drupalSettings.entity_browser[this.uuid];this.count=settings.count||0;this.cardinality=settings.cardinality||1;},deselect:function(item){this.$(item).removeClass('checked').find('input[name ^= "entity_browser_select"]').prop('checked',false);},deselectAll:function(){var _deselect=jQuery.proxy(this.deselect,this);this.$('.views-row').each(function(undefined,item){_deselect(item);});},select:function(item){this.$(item).addClass('checked').find('input[name ^= "entity_browser_select"]').prop('checked',true);},lock:function(){this.$('.views-row:not(.checked)').addClass('disabled');},unlock:function(){this.$('.views-row').removeClass('disabled');},warn:function(){var text=Drupal.t('Youʼve reached the maximum selection limit. Please deselect an item in order to choose another.');var $message=$('<div class="media-browser-limit"></div>').text(' '+text);if(!this.$('.media-browser-file-counter .media-browser-limit').length)this.$('.media-browser-file-counter').addClass('messages--warning').append($message);},unwarn:function(){this.$('.media-browser-limit').each(function(){$(this).remove();});this.$('.media-browser-file-counter').removeClass('messages--warning').addClass('messages--status');},onClick:function(event){var chosen_one=this.$(event.currentTarget);if(chosen_one.hasClass('disabled')){this.warn();return false;}else if(this.cardinality===1){this.deselectAll();this.select(chosen_one);}else if(chosen_one.hasClass('checked')){this.deselect(chosen_one);this.count--;this.unlock();this.unwarn();this.renderCounter();}else{this.select(chosen_one);if(++this.count===this.cardinality)this.lock();this.renderCounter();}}});Drupal.behaviors.mediaEntityBrowserView={getElement:function(context){return $(context)[context===document?'find':'filter']('form[data-entity-browser-uuid]').not('.dxpr-builder-media-processed').addClass('dxpr-builder-media-processed').get(0);},attach:function(context){var element=this.getElement(context);if(element)$(element).data('view',new Selection({el:element}));},detach:function(context){var element=this.getElement(context);if(element){var view=$(element).data('view');if(view instanceof Selection)view.undelegateEvents();}}};})(jQuery,_,Backbone,Drupal);;
(function($,Drupal,drupalSettings,once){'use strict';Drupal.behaviors.entityBrowserView={attach:function(context){var views_instance=Drupal.views.instances[Object.keys(Drupal.views.instances)[0]];if(views_instance){views_instance.$exposed_form=$('.js-view-dom-id-'+views_instance.settings.view_dom_id+' .views-exposed-form');$(once('exposed-form',views_instance.$exposed_form)).each(jQuery.proxy(views_instance.attachExposedFormAjax,views_instance));if(views_instance.exposedFormAjax&&views_instance.exposedFormAjax.length>0){var ajax=views_instance.exposedFormAjax[0];ajax.options.beforeSubmit=function(form_values,element_settings,options){form_values=form_values.splice(form_values.length-3,3);ajax.ajaxing=true;return ajax.beforeSubmit(form_values,element_settings,options);};}$(once('submit-by-enter-key',views_instance.$exposed_form)).find('input[type="text"]').each(function(){$(this).on('keypress',function(event){if(event.keyCode==13){event.preventDefault();views_instance.$exposed_form.find('input[type="submit"]').first().click();}});});if(drupalSettings.entity_browser_widget.auto_select){var selection_cells=views_instance.$view.find('.views-field-entity-browser-select');if(selection_cells.length>0){$(once('register-row-click',selection_cells.parent())).click(function(event){event.preventDefault();var $row=$(this);var $input=$row.find('.views-field-entity-browser-select input.form-checkbox, .views-field-entity-browser-select input.form-radio');$row.parents('form').find('.entities-list').trigger('add-entities',[[$input.val()]]);});selection_cells.hide();}}}}};}(jQuery,Drupal,drupalSettings,once));;
(function($,Drupal,drupalSettings,once){'use strict';Drupal.behaviors.entityBrowserTabs={attach:function(context){var $form=$(once('entity-browser-admin','.entity-browser-form',context));if(!$form.length)return;var $nav=$('<nav class="eb-tabs"></nav>');var $tabs=$(Drupal.theme('entityTabs'));$form.find('.tab').each(function(index,element){var $element=$(element);var classesArray=[];classesArray.push($element.attr('disabled')?'is-active active':'');var text=$element.text();var textClass=text.toLowerCase().replace(/ /g,'-');classesArray.push(textClass);var classes=classesArray.join(' ');var tabSettings={class:classes,id:$element.attr('id'),title:$(this)[0].value};var $tab=$(Drupal.theme('entityTab',tabSettings));$tab.find('a').on('click',function(event){var buttonID=$(event.currentTarget).data().buttonId;$form.find('#'+buttonID).trigger('click');event.preventDefault();});$tab.appendTo($tabs);});$tabs.appendTo($nav);$nav.prependTo($form);$form.find('.tab').css('display','none');}};Drupal.theme.entityTabs=function(){return $('<ul role="navigation" aria-label="Tabs"></ul>');};Drupal.theme.entityTab=function(settings){return $('<li tabindex="-1"></li>').addClass(settings.class).append($('<a href="#"></a>').addClass(settings.class).attr('data-button-id',settings.id).append(settings.title));};}(jQuery,Drupal,drupalSettings,once));;
